...
def train(model, num_epochs, train_loader, optimizer, start_epoch):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        batch_iterator = tqdm(train_loader, desc=f"Training,epoch={epoch+1}", leave=True)
...
if __name__ == '__main__':
    ...
    IMG_HEIGHT = 512
    # Load data
    transform = T.Compose([T.ToTensor(), T.RandomHorizontalFlip(), T.RandomVerticalFlip(), T.Resize((IMG_HEIGHT, IMG_WIDTH), antialias=True)])
    train_dataset = CustomDataset('data', 'data_mask', transform=transform)
    val_dataset = CustomDataset('test', 'test_mask', transform=transform)
