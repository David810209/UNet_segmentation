...
def evaluate_model(model, test_loader, output_dir="output_images", device="cuda"):
    ...
    with torch.no_grad():
        for idx, (x, y, filename) in enumerate(
            ...
            # concat output and y
            img = torch.cat((output, y), dim=3)[0]
            img_pil = T.ToPILImage()(img.cpu())
            img_pil.save(os.path.join(output_dir, f"concat_{filename[0]}"))
            ...
            # 打印輸出範圍
            # print(
            #     f"Batch {idx}: Output Min: {output.min().item()}, Output Max: {output.max().item()}, Output Mean: {output.mean().item()}"
            # )
...
def combine_images(input_folder, label_folder, model_output_folder, output_folder):
    ...
    input_images = sort_nicely(
        [f for f in os.listdir(input_folder) if f.endswith(".jpg")]
    )
    label_images = sort_nicely(
        [f for f in os.listdir(label_folder) if f.endswith(".jpg")]
    )
    model_output_images = sort_nicely(
        [f for f in os.listdir(model_output_folder) if f.endswith(".jpg")]
    )
    ...
    for i in range(num_images):
        ...
        model_output_image_path = os.path.join(
            model_output_folder, model_output_images[i]
        )
        ...
        if input_image is None or label_image is None or model_output_image is None:
            print(
                f"無法讀取以下圖像之一: {input_image_path}, {label_image_path}, {model_output_image_path}"
            )
            continue
        ...
        input_image_resized = cv2.resize(
            input_image, (target_width, target_height), interpolation=cv2.INTER_LANCZOS4
        )
        label_image_resized = cv2.resize(
            label_image, (target_width, target_height), interpolation=cv2.INTER_LANCZOS4
        )
        model_output_image_resized = cv2.resize(
            model_output_image,
        ...
        def pad_image(image, target_size=(840, 820), color=(255, 255, 255)):
            padded_image = np.full(
                (target_size[1], target_size[0], 3), color, dtype=np.uint8
            )
        ...
        combined_image = np.hstack(
            (input_image_padded, label_image_padded, model_output_image_padded)
        )
